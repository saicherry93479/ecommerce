...
export default function ProductDetails({ 
  ...
  const handleWishlistToggle = async () => {
    try {
      if (isInWishlist) {
        await axios.post('http://13.233.133.71:5000/api/wishlist/remove', { productId: product._id });
        setWishlist(wishlist.filter(item => item._id !== product._id));
      } else {
        await axios.post('http://13.233.133.71:5000/api/wishlist/add', { productId: product._id });
